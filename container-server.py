...
def servies_index():
    ...
    services = docker_ps_to_array(docker('service', 'ls') )
    resp = json.dumps(services)
    return Response(response=resp, mimetype="application/json")
...
def nodes_index():
    ...
    """
    nodes = docker_node_to_array(docker('node', 'ls') )
    
    resp = json.dumps(nodes)
    return Response(response=resp, mimetype="application/json")
...
#
# Parses the output of the Docker  command to a  node.
#
